"""Communication primitives for venue-meetup agents."""
...
class CommsRouter:
    ...
    def resolve_recipients(
        ...
        if message.sender not in agent_ids:
            errors.append(CommsError(step=message.step, sender=message.sender, recipients=message.recipients, error="unknown sender"))
            return [], errors
        ...
        for recipient in message.recipients or []:
            if recipient == message.sender:
                errors.append(
                    CommsError(step=message.step, sender=message.sender, recipients=message.recipients, error=f"self recipient dropped: {recipient}")
                )
                continue
            if recipient not in agent_ids:
                errors.append(
                    CommsError(step=message.step, sender=message.sender, recipients=message.recipients, error=f"unknown recipient dropped: {recipient}")
                )
                continue
...
class BroadcastRouter(CommsRouter):
    ...
    def resolve_recipients(
        ...
        """Return every teammate as recipient."""
...
class DirectedRouter(CommsRouter):
    """Route messages to validated recipients, with broadcast fallback."""
...
class ProximityRouter(DirectedRouter):
    ...
    def resolve_recipients(
        ...
        """Return recipients that are in communication range."""
        ...
        for recipient in recipients:
            ...
            if distance <= self.radius:
                ...
                errors.append(
                    CommsError(step=message.step, sender=message.sender, recipients=message.recipients, error=f"recipient out of range: {recipient}")
                )
...
class MessageBus:
    ...
    def __init__(self, agent_ids: list[str], router: CommsRouter | None = None, max_history: int = 12):
    ...
    def deliver(self, messages: list[Message], positions: dict[str, tuple[float, float]] | None = None) -> dict[str, list[Message]]:
...
def messages_from_turns(turns: dict[str, Any], *, step: int, default_recipients: list[str] | None = None) -> list[Message]:
